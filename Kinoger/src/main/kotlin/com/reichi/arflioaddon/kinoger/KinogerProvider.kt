...
class KinogerProvider : TmdbProvider() {
    ...
    /**
     * supervideo.cc extractor. Embed page (/e/<mediaId>) packs the JWPlayer config in a
     * Dean-Edwards p.a.c.k.e.r eval block (this variant passes a and c as BARE integers).
     * Unpack it and read the m3u8 from jwplayer setup sources. No bot protection (HTTP 200).
    ...
    private fun resolveSupervideo(url: String, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val res = httpGet(url, headers = mapOf("Referer" to ref, "User-Agent" to desktopUA))
        if (res.code !in 200..299) {
        ...
        if (fileUrl.isNullOrEmpty() || !fileUrl.startsWith("http")) {
            DebugLog.w(dbg, "resolveSupervideo: no sources file URL")
            return false
        }
    ...
    /**
     * Dean-Edwards p.a.c.k.e.r unpacker. Finds eval(function(p,a,c,k,e,d){...}('p',A,C,'k'.split('|')))
     * (this variant passes A and C as BARE integers, e.g. 36,381), reverses the token substitution,
     * returns the unpacked JS. Returns "" if no packer block is found.
     */
